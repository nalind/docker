...
test_env() {
	...
	env -i \
		DEST="$DEST" \
		${DOCKER_AUTHN_PASSWORD:+DOCKER_AUTHN_PASSWORD="$DOCKER_AUTHN_PASSWORD"} \
		${DOCKER_AUTHN_USERID:+DOCKER_AUTHN_USERID="$DOCKER_AUTHN_USERID"} \
		${DOCKER_BEARER_TOKEN:+DOCKER_BEARER_TOKEN="$DOCKER_BEARER_TOKEN"} \
		...
		${DOCKER_DAEMON_ARGS:+DOCKER_DAEMON_ARGS="$DOCKER_DAEMON_ARGS"} \
		GOPATH="$GOPATH" \
