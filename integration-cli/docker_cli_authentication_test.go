...
func (s *DockerAuthnSuite) AuthenticateBasic(w http.ResponseWriter, r *http.Request) {
	req := authentication.AuthnPluginRequest{URL: make(map[string]string), Options: make(map[string]string)}
	resp := authentication.AuthnPluginResponse{Header: make(http.Header)}
	w.Header().Set("Content-Type", "application/vnd.docker.plugins.v1+json")
	...
	realm, ok := req.Options["realm"]
	if !ok {
		realm = "localhost"
	}
	headers := req.Header[http.CanonicalHeaderKey("Authorization")]
	if len(headers) == 0 {
		resp.Header.Add("WWW-Authenticate", "Basic "+realm)
	} else {
...
func (s *DockerAuthnSuite) AuthenticateProxy(w http.ResponseWriter, r *http.Request) {
	...
	proxyHeaderName := req.Options[s.proxyHeaderNameOption]
...
func (s *DockerAuthnSuite) TestBasicAuthnBad1(c *check.C) {
	...
	c.Assert(strings.TrimSpace(stdout) == "authentication failed for request", check.Equals, true, check.Commentf("actual output is: %q (stderr is %q)", stdout, stderr))
}
...
func (s *DockerAuthnSuite) TestProxyAuthnBad(c *check.C) {
	...
	c.Assert(strings.TrimSpace(stdout) == "unauthenticated request rejected", check.Equals, true, check.Commentf("actual output is: %q (stderr is %q)", stdout, stderr))
}
