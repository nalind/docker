...
func (n *negotiate) authStep(cli *Client, challenge string, req *http.Request) (result bool, err error) {
	...
	if strings.ToLower(fields[0]) == "negotiate" {
		...
		if otoken.length > 0 {
			if req == nil {
				logrus.Errorf("synchronization error: got unexpected Negotiate token to send to server")
				return false, nil
			}
...
func (n *negotiate) authRespond(cli *Client, challenge string, req *http.Request) (result bool, err error) {
	if !cli.authnOpts.negotiate {
		return false, nil
	}
	if n.hostname == "" {
		serverhost := req.Host
...
func (n *negotiate) authCompleted(cli *Client, challenge string, resp *http.Response) (completed bool, err error) {
	var minor C.OM_uint32
	if !cli.authnOpts.negotiate {
		return false, nil
	}
	completed, err = n.authStep(cli, challenge, nil)
