...
func (a *Authentication) makeReq(r *http.Request) *AuthnPluginRequest {
...
func makeResp() AuthnPluginResponse {
	return AuthnPluginResponse{Header: make(http.Header)}
}
...
func (a *Authentication) callPlugins(w http.ResponseWriter, r *http.Request) (User, error) {
	...
	for _, pname := range a.pluginNames {
		...
		resp := makeResp()
		if err := plugin.Client.Call(AuthenticationRequestName, &req, &resp); err != nil {
...
// "Authentication" interface, which consists of a single entry point:
//    /Authentication.Authenticate
// It receives an AuthnPluginRequest and returns an AuthnPluginResponse.
//
...
func NewAuthentication(required bool, options map[string]string, makeAuthError func(error) error) *Authentication {
	...
	return &Authentication{pluginNames: names, plugins: handles, options: options, required: required, makeAuthError: makeAuthError}
}
